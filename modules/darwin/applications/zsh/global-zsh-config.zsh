...
#  zshrc
#
[ -f "/etc/zshplugins.local" ] && source "/etc/zshplugins.local"
...
function get_start_time() {
  _did_run_cmd=1
  cmd_timer=$EPOCHREALTIME  # float: seconds.microseconds
}

function calc_exec_time() {
  if [[ -n $cmd_timer ]]; then
    local now=$EPOCHREALTIME
    local -i d_ms=$(( ($now - $cmd_timer) * 1000 ))
    local -i d_s=$(( d_ms / 1000 ))
    local -i ms=$(( d_ms % 1000 ))
    local -i s=$(( d_s % 60 ))
    local -i m=$(( (d_s / 60) % 60 ))
    local -i h=$(( d_s / 3600 ))
...
function _update_prompt_status() {
  ...
  LAST_RC=$rc
  if (( rc == 0 )); then
    PROMPT_STATUS_STRING="%F{yellow}(rc: %F{green}0%F{yellow} et: %F{cyan}${elapsed}%F{yellow})%f"
  else
    PROMPT_STATUS_STRING="%F{yellow}(rc: %B%F{red}${rc}%b%F{yellow} et: %F{cyan}${elapsed}%F{yellow})%f"
  fi
  PROMPT_STATUS_STRING=${(%)PROMPT_STATUS_STRING}
  export PROMPT_STATUS_STRING LAST_RC
}
...
%{$fg[yellow]%}(rc: ${LAST_RC} et: ${elapsed} )%{$reset_color%}'
...
# Handy ls aliases
alias ls='eza --group --color=always --icons=always --git'
alias ll='ls -lF --group'
alias la='ls -la'
alias lt='ls -lt modified --sort modified --group'
alias l='ls -CF'
...
alias rbd='nh darwin switch $(readlink -f /etc/nix-darwin)'
alias rbh='nh home switch ~/.config/home-manager'
alias mx='manix "" | grep "^# " | sed "s/^# \(.*\) (.*/\1/;s/ (.*//;s/^# //" | fzf --preview="manix {}" | xargs manix'
alias ns='nix-search-tv print | fzf --preview "nix-search-tv preview {}" --scheme history | cut -d " " -f 2'
alias nco='nh clean all --keep 30 && nix store optimise'

# journald failures
alias lastfail='notify-send --urgency=low "Last failure:" "$(journalctl --no-pager -e | grep failed | tail -n1)"'
alias watchdog='watch -n 60 journalctl --since "1 hour ago" | grep -i failed | tail -n 10'

# get weather for Jackson, MI airport
alias weather='curl "wttr.in/jxn?2Qu"'
